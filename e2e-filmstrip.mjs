...
// The tabs themselves. The split toggle and the zoom readout share the row with
// them and are not tabs, so they are not counted here.
const tabsBefore = await page.evaluate(() =>
  [...document.querySelectorAll('.tl-tabs button.tl-tab')].map((b) => b.textContent.trim()))
